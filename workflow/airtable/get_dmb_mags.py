### Script for grabbing MAG inputs for the EHIO profiling pipeline
### This pulls MAG URLs from the 'MAG' table.
...
query_params = {
    'filterByFormula': f"AND(FIND('{args.dmb}', {{DM_batch}}), NOT({{phylum}} = 'nan'))",
...
with open(output_file_path, 'w', newline='') as tsvfile:
    ...
    writer.writerow(['genome', 'completeness', 'contamination', 'url'])
    ...
    while True:
        ...
        for record in records:
            ...
            url = record_response.json()['fields'].get('MAG_url', '')

            # Write the row to the CSV file
            row = [mag_name, completeness, contamination, url]
            writer.writerow(row)
