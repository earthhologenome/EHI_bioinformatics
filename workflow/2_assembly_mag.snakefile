...
def estimate_time_binning(wildcards, attempt):
    ...
    C = row["C"]
    gbp_post_mapping = calculate_input_size_gb(row["metagenomic_bases"])
    estimate_time_binning = -595.79 + (25.59 * diversity) - (0.18 * singlem_fraction) + (9 * gbp_post_mapping) + (169.88 * C)
...
include: os.path.join(config["codedir"], "rules/maxbin2.smk")
include: os.path.join(config["codedir"], "rules/metabat2.smk")
include: os.path.join(config["codedir"], "rules/concoct.smk")
