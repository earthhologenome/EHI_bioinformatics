...
SAMPLES = [os.path.relpath(fn, "2_Reads/4_Host_removed").replace("_M_1.fastq.gz", "")
            for group in GROUP
...
rule Coassembly_index:
    ...
    shell:
        ...
        """
################################################################################
### Create checkpoint for sample wildcard
checkpoint get_r1_wildcard:
    input:
...
def get_r1_wildcard(wildcards):
    checkpoint_output = checkpoints.get_r1_wildcard.get(**wildcards).output[0]
    file_names = expand("2_Reads/4_Host_removed/{group}/samples/{r1}_M_1.fastq.gz",
                        group = GROUP, r1 = glob_wildcards(os.path.join(checkpoint_output, "{r1}._M_1.fastq.gz")).r1)
    return file_names

def get_r2_wildcard(wildcards):
    checkpoint_output = checkpoints.get_r2_wildcard.get(**wildcards).output[0]
    file_names = expand("2_Reads/4_Host_removed/{group}/samples/{r2}_M_2.fastq.gz",
                        group = GROUP, r2 = glob_wildcards(os.path.join(checkpoint_output, "{r2}._M_2.fastq.gz")).r2)
    return file_names
...
rule Coassembly_mapping:
    input:
        ...
        bt2_index = "3_Outputs/2_Coassemblies/{group}/{group}_contigs.fasta.rev.2.bt2l"
    output:
        "3_Outputs/3_Coassembly_Mapping/BAMs/{group}/output.txt"
    params:
...
rule metabinner:
    ...
    output:
        ...
        bins_path = directory("3_Outputs/4_Binning/{group}/metabinner_bins/metabinner_res/ensemble_res/greedy_cont_weight_3_mincomp_50.0_maxcont_15.0_bins/ensemble_3logtrans/addrefined2and3comps/greedy_cont_weight_3_mincomp_50.0_maxcont_15.0_bins")
    params:
        minlength = expand("{minlength}", minlength=config['minlength']),
        bams = "3_Outputs/3_Coassembly_Mapping/BAMs/{group}",
